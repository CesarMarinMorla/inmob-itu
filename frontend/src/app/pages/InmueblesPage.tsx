...
const TIPO_LABEL: Record<TipoProp, string> = {
  casa: 'Casa',
...
export default function InmueblesPage() {
  ...
  const filtradas = filtro === 'todos'
    ? propiedades
    : propiedades.filter((p) => p.tipoProp === filtro);
  ...
  const handleEliminar = async () => {
    ...
    setDeleting(true);
    try {
      if (deleteTarget.tipoProp === 'casa') await deleteCasa(deleteTarget.id);
      else if (deleteTarget.tipoProp === 'departamento') await deleteDepartamento(deleteTarget.id);
      else await deleteTerreno(deleteTarget.id);
  ...
  return (
    <Box>
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  <Box sx={{ mb: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    <Chip
                      label={ESTADO_LABEL[prop.estado] ?? prop.estado}
                      size="small"
                      color={ESTADO_COLOR[prop.estado] ?? 'default'}
                    />
                    <Chip
                      label={TIPO_LABEL[prop.tipoProp]}
                      size="small"
                      variant="outlined"
                    />
                  </Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {formatDireccion(prop)}
                  </Typography>

                  {prop.superficieTotal && (
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                      Superficie: {prop.superficieTotal} m²
                    </Typography>
                  )}
                </CardContent>
                ...
                <CardActions sx={{ gap: 1, px: 2, pb: 2 }}>
                  <Button
                    size="small"
                    variant="outlined"
                    onClick={() => navigate(`/inmuebles/${prop.tipoProp}/${prop.id}/editar`)}
                  >
                    Editar
                  </Button>
                  <Button
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
        ...
        <DialogContent>
          <DialogContentText>
            ¿Estás seguro de eliminar <strong>{deleteTarget?.codigoRef}</strong>? Esta acción no se puede deshacer.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteTarget(null)} disabled={deleting}>Cancelar</Button>
          <Button onClick={handleEliminar} color="error" disabled={deleting} variant="contained">
