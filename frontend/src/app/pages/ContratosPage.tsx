...
export default function ContratosPage() {
  ...
  const handleEliminar = async () => {
    ...
    setDeleting(true);
    try {
      await deleteContrato(deleteTarget.id);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<Add />} onClick={() => navigate('/contratos/nuevo')}>
          Nuevo Contrato
        </Button>
      </Box>
                ...
                <TableRow key={c.id} hover>
                  ...
                  <TableCell>
                    <Chip
                      ...
                      size="small"
                      color={ESTADO_COLOR[c.contratoEstado]}
                    />
                  </TableCell>
                  <TableCell>
                    {c.montoBase.toLocaleString('es-AR')} {c.tipoMoneda}
                  </TableCell>
                  <TableCell align="right">
                    <IconButton size="small" onClick={() => navigate(`/contratos/${c.id}/editar`)}>
                      <Edit fontSize="small" />
                    </IconButton>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
        ...
        <DialogContent>
          <DialogContentText>
            ¿Eliminás el contrato <strong>{deleteTarget?.contratoNumero}</strong>? Esta acción no se puede deshacer.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
